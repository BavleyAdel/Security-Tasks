...
public class PlayfairCipher {
    ...
    public String decrypt(String text) {
        ...
        for (int i = 0; i < text.length()-1; i += 2) {
            ...
            if (pos1[0] == pos2[0]) {  // Same row
//                if(checkX(text,keyMatrix[pos1[0]][(pos1[1] + 4) % 5],i))
                    decryptedText.append(keyMatrix[pos1[0]][(pos1[1] + 4) % 5]);
//                if(checkX(text,keyMatrix[pos2[0]][(pos2[1] + 4) % 5],i))
                    decryptedText.append(keyMatrix[pos2[0]][(pos2[1] + 4) % 5]);
            } else if (pos1[1] == pos2[1]) {  // Same column
//                if(checkX(text,keyMatrix[(pos1[0] + 4) % 5][pos1[1]],i))
                    decryptedText.append(keyMatrix[(pos1[0] + 4) % 5][pos1[1]]);
//                if(checkX(text,keyMatrix[(pos2[0] + 4) % 5][pos2[1]],i))
                    decryptedText.append(keyMatrix[(pos2[0] + 4) % 5][pos2[1]]);
            } else {  // Rectangle swap
//                if(checkX(text,keyMatrix[pos1[0]][pos2[1]],i))
                    decryptedText.append(keyMatrix[pos1[0]][pos2[1]]);
//                if(checkX(text,keyMatrix[pos2[0]][pos1[1]],i))
                    decryptedText.append(keyMatrix[pos2[0]][pos1[1]]);
            }
        }

        return checkX(decryptedText.toString());
    }

    public String checkX(String decryptedText) {
        decryptedText = decryptedText.toUpperCase();
        StringBuilder dt = new StringBuilder();

        for (int i = 0; i < decryptedText.length(); i++) {
            dt.append(decryptedText.charAt(i));
            // remove 'X' if two consecutive letters are the same
            if (i > 1 && i < decryptedText.length() - 1 && decryptedText.charAt(i) == 'X'
                    && decryptedText.charAt(i - 1) == decryptedText.charAt(i + 1) && i % 2 != 0) {
                dt.deleteCharAt(i);
            }
        }
        // remove last x
        if (dt.length() > 0 && dt.charAt(dt.length() - 1) == 'X') {
            dt.deleteCharAt(dt.length() - 1);
        }
        decryptedText = dt.toString();
